...
// sẽ bị xóa trong cập nhật tới
//std::string thuc_hien_http_request(const thamso_http& ts)
//{
...
//	}
//
//	std::string url = ts.url.substr(strlen("https://"));
//	auto pos = url.find('/');
...
//	std::string target = url.substr(pos);
//
//	try
//	{
//		net::io_context ioc;
...
//		if (ts.method == "POST")
//		{
//			req = http::request<http::string_body>{ http::verb::post, target, 11 };
//			req.body() = ts.body;
//			req.prepare_payload();
//		}
//		else
...
//		else
//			req.set(http::field::accept, ts.accept);
//
...
//		auto& res = parser.get();
//
//		if (ts.cho_phep_redirect &&
//			(res.result() == http::status::moved_permanently || res.result() == http::status::found || res.result() == http::status::temporary_redirect ||
...
//			if (it != res.end())
//			{
//				thamso_http ts_moi = ts;
//				ts_moi.url = std::string(it->value().data(), it->value().size());
//				ts_moi.so_lan_redirect += 1;
//				return thuc_hien_http_request(ts_moi);
//			}
//
...
//		{
//			td_log(loai_log::loi, "HTTP lỗi: " + std::to_string(static_cast<int>(res.result())));
//			return "";
//		}
//
//		beast::error_code ec;
...
//	{
//		td_log(loai_log::loi, "HTTP request exception: " + std::string(e.what()));
//		return "";
//	}
//}
//
...
//	cauhinh_api ap;
//	thamso_http ts;
//	ts.url = "https://" + ap.host + ap.target;
//	ts.method = "POST";
//	ts.body = noidung_json;
//	ts.user_agent = "ZitApp/1.0";
//	ts.accept = ap.accept;
//	ts.cho_phep_redirect = false;
//	return thuc_hien_http_request(ts);
//}
//
...
//				ts_moi.url = std::string(it->value().data(), it->value().size());
//				ts_moi.so_lan_redirect += 1;
//				return gui_http_request(ts_moi);
//			}
//
//			td_log(loai_log::loi, "Không tìm thấy trường 'Location' trong phản hồi redirect.");
//			return "";
...
//	cauhinh_github gh;
//	const std::string target = "/repos/" + gh.owner + "/" + gh.repo_tainguyen + "/contents/" + file_path;
//	std::string response = tai_raw_github_file(target);
//	if (response.empty())
//		td_log(loai_log::loi, "Không thể tải dữ liệu từ GitHub");
//	return response;
//}
...
//{
//	const std::string url = "https://api.github.com/repos/eTog205/zitapp/releases/latest";
//	const std::string response_body = tai_url_http(url, false);
//	if (response_body.empty())
//	{
//		td_log(loai_log::loi, "[get_release_tag] Không nhận được phản hồi từ GitHub API!");
//		return "";
...
//
//	json::value jv = json::parse(response_body);
//	if (!jv.is_object() || !jv.as_object().contains("tag_name") || !jv.as_object()["tag_name"].is_string())
//	{
//		td_log(loai_log::canh_bao, "Không tìm thấy 'tag_name' trong phản hồi JSON.");
//		return "";
//	}
...
//	body_obj["key"] = key;
//
//	std::string res = gui_post_api(serialize(body_obj));
//	if (res.empty())
//		return std::nullopt;
//
//	try
